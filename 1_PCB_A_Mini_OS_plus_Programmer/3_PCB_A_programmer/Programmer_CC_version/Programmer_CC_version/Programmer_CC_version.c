...
/********  PCB-A programmer: runs in the boot space of the PCB_A ATMEGA328**********/
...
PCB_A_programmer is used to program the UNO device with its bootloader, verification and the default user projects
...
0x3FB		Controls multiplexer (T0) on interval:  0xFF for 2ms, 0xFE for 500uS and 0xFD for 125uS
...
if ((waitforkeypress()) != 'p')							//For keypress h,t or r
...
Read_write_mem('I', 0x3EF, 0);						//Write UNO EEPROM 0x3FE to zero: Offset for PRN EEPROM registers//////////
...
start_new_code_block(); 											//Initialise new programming block (usually starts at address zero but not exclusive so)
